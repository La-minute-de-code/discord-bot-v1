...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('classement')
        .setDescription('Affiche le classement des utilisateurs par points exp'),
    ...
    async execute(interaction) {
        ...
        async function generateEmbed(pageNumber) {
            ...
            const embed = new EmbedBuilder()
                .setTitle('🏆 Classement EXP')
                .setColor('#4D177C')
                .setDescription(
                    users.map((user, index) => {
                        const position = skip + index + 1;
                        const medal = position === 1 ? '🥇' : position === 2 ? '🥈' : position === 3 ? '🥉' : '▫️';
                        return `${medal} **#${position}** - ${user.username}\n▸ EXP: ${user.exp.toLocaleString()} points`;
                    }).join('\n\n')
                )
                .setFooter({ text: `Page ${pageNumber}/${totalPages} • ${totalUsers} utilisateurs au total` })
                .setTimestamp();
